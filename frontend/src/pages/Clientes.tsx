...
type AuditEntry = { id: number; accion: string; tabla?: string; registro_id?: number; detalles?: Record<string, string | number | null>; created_at: string };
...
export default function ClientesPage({ activeView = 'clientes', onSelectView = () => undefined }: PageProps) {
  ...
  const historyTelemetry = useMemo(() => {
    ...
    const actionCounts = countBy(history.map((entry) => entry.accion));
    const tableCounts = countBy(history.map((entry) => entry.tabla ?? 'sin tabla'));
    return { actionCounts, tableCounts };
  }, [history]);
